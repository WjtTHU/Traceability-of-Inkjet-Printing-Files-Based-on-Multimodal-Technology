...
def baseline(df):
    ...
    # X = train_df.drop(labels=["Label", "ID", "Color_1", "Color_2", "Color_3", "Color_4"], axis=1)
    Column = spectra_data.drop(labels=["Label", "ID", "Color_1", "Color_2", "Color_3", "Color_4", 'peaks_count', 'max_peak_position', 'std_deviation','total_integral', 'valleys_count', 'min_peak_position',
                'skewness', 'kurtosis', 'entropy', 'gini_coefficient'], axis=1)
    ...
    processed_df = pd.DataFrame(baseline_corrected_data, columns=Column.columns)
    ...
    return df
    # return processed_df #效果可能不如不做基线调整
